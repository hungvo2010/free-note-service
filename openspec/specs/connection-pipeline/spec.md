...
Define how ConnectionPipeline manages connection lifecycle and handler orchestration in a transport-agnostic way, returning keepAlive signals to the transport layer.
...
`ConnectionPipeline` SHALL track each connection's lifecycle state (`HandShakeState` to `MessageState` to close) in an internal map keyed by `NetworkRequestData`. When `process()` returns `false`, the transport SHALL close the channel and the pipeline SHALL have already removed the state.
