...
Define how the `@WebSocketEndpoint` annotation, its two annotation processors (`WebServerProcessor` for validation, `BeanProviderProcessor` for code generation), and the generated `URIHandlerRegistry` work together to enable compile-time WebSocket route discovery and runtime dispatch.
